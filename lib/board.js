...
Board = (function(_super) {
  ...
  function Board(config) {
    var GpioDriver, SerialPortDriver, _ref;
    this.config = config;
    this._waitForAcknowledge = __bind(this._waitForAcknowledge, this);
    this._onAcknowledge = __bind(this._onAcknowledge, this);
    assert((_ref = this.config.driver) === "serialport" || _ref === "gpio");
    switch (this.config.driver) {
      case "serialport":
        SerialPortDriver = require('./driver/serialport');
        this.driver = new SerialPortDriver(this.config.driverOptions);
        break;
      case "gpio":
        GpioDriver = require('./driver/gpio');
        this.driver = new GpioDriver(this.config.driverOptions);
    }
  ...
  Board.prototype.writeAndWait = function(data) {
    return this._lastAction = settled(this._lastAction).then((function(_this) {
      return function() {
        _this.emit('debugSend', data.replace(/\r?\n|\r/, ""));
